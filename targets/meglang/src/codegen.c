...
typedef struct Emitter { FILE *out; DiagnosticSink diagnostics; bool failed; } Emitter;
...
static bool emit_expr(Emitter *emitter, const Expr *expr)
{
    ...
    switch (expr->kind) {
    case EXPR_INT: return emit(emitter, "INT64_C(%lld)", (long long)expr->as.integer);
...
bool codegen_c(FILE *out, const Program *program, DiagnosticSink diagnostics)
{
    ...
    if (!emit(&emitter,
        ...
        "    if (b == 0 || (a == INT64_MIN && b == -1)) abort();\n"
        "    return a %% b;\n}\n\n"
        "static int64_t meg_main(void) ")) return false;
